...
export function isCompressibleImagePath(file) {
    return /\.(?:png|jpe?g|webp|gif)$/i.test(file);
}
export function resolveCompressionFormat(format, inFile) {
...
export function buildGifCompressionArgs(inFile, outFile) {
    return [
        '-y',
        '-v',
        'error',
...
export async function compressSharp(image, inBuffer, inFile, outFile) {
...
export async function compressGif(inFile, outFile) {
...
export async function compressImages(files) {
    await Promise.all(files.map(async (file) => {
        ...
        const { percent, size, outSize, inFile, outFile, outBuffer, } = compression;
        if (percent > -0.10) {
