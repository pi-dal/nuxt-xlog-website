...
function findSocialLink(links, candidates) {
    if (!links?.length)
        return undefined;
...
export function resolveSiteUrl(site, fallbackHandle = 'pi-dal') {
...
export function buildCollectionPageHead(options) {
...
export function buildContentPageHead(options) {
    const url = buildCanonicalUrl(options.path);
    const title = buildPageTitle(options.title);
...
export function resolveAuthorHandle(site) {
