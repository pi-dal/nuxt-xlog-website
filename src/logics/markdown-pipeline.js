...
const OBSIDIAN_CALLOUT_ALIASES = {
    abstract: 'note',
};
export function convertIpfsUrls(content) {
    if (!content)
        return content;
...
export function normalizeObsidianCalloutAliases(content) {
    if (!content)
        return content;
...
export function buildMarkdownPipelineConfig(options = {}) {
    const transformers = [
...
export function applyMarkdownImagePlugin(md) {
    ...
    md.renderer.rules.image = (tokens, idx, options, env, self) => {
        ...
        const attrs = (token.attrs || []).map(([k, v]) => `${k}="${v}"`);
        const hasSrc = token.attrs?.some((a) => a[0] === 'src');
        if (!hasSrc)
...
export async function applyMarkdownPipeline(md, options = {}) {
