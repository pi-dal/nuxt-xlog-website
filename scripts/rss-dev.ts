...
async function run() {
  console.log('🚀 Starting RSS generation for development...')
  ...
  const manifest = await loadManifest<RssManifestEntry>('rss-manifest-dev.json')
  ...
  await saveManifest('rss-manifest-dev.json', nextManifest)
...
async function writeFeed(name: string, options: FeedOptions, items: Item[], context: FeedContext) {
  ...
  console.log(`📝 Generated RSS file: public/${name}.xml`)

  if (name === 'feed') {
    await fs.ensureDir('./public/zh')
    await fs.writeFile('./public/zh/index.xml', feed.rss2(), 'utf-8')
    console.log('📝 Generated compatibility RSS file: public/zh/index.xml')
  }
}
